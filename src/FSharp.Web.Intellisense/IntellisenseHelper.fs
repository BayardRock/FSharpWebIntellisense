...
let GetDeclarations (source:string) (lineIndex, charIndex) =
    ...
    let identifier = extractIdentifier line charIndex
    ...
    let decls = 
        parsed.GetDeclarations(Some untyped, (lineIndex, charIndex), source, ([identifier], ""), fun _ -> false)
...
let GetMethods (source:string) (lineIndex, charIndex) =
    ...
    let methods = parsed.GetMethods((lineIndex, charIndex), inputLines.[lineIndex], None)
...
let GetToolTip (source:string) (lineIndex, charIndex) =
    ...
    parsed.GetDataTipText((lineIndex, charIndex), line, [identifier], identToken)
