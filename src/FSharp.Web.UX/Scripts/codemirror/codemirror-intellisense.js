...
"}";
    ...
    self.showAutoComplete = function(b)
    {
    	if (b)
    	{
    		editor.addKeyMap(intellisenseKeyMap);
    	} 
    	else
		{
    		editor.removeKeyMap(intellisenseKeyMap);
		}
	...
	var intellisenseKeyMap = 
	{
        "Up": function (ed) { self.moveAutoComplete(-1); },
        "Down": function (ed) { self.moveAutoComplete(1); },
        "Left": function (ed) { self.showAutoComplete(false); },
        "Right": function (ed) { self.showAutoComplete(false); },
        "PageUp": function (ed) { self.moveAutoComplete(-5); },
        "PageDown": function (ed) { self.moveAutoComplete(5); },
        "Enter": function (ed) { self.insertAutoComplete(); },
        "Tab": function (ed) { self.insertAutoComplete(); }
	};
